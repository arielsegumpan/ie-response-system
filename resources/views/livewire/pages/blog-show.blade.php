          ...
          <div class="space-y-5 lg:space-y-8">
            ...
            <div class="flex items-center gap-x-5">
                @foreach ($blog->categories as $category)
                    <a href="#" class="inline-flex items-center gap-1.5 py-1 px-3 rounded-full text-xs border border-transparent bg-red-100 text-red-800 hover:bg-red-200 focus:outline-hidden focus:bg-red-200 disabled:opacity-50 disabled:pointer-events-none dark:text-red-500 dark:bg-red-800/30 dark:hover:bg-red-800/20 dark:focus:bg-red-800/20">
                    {{ $category->cat_name }}
                    </a>
                @endforeach
          ...
          <div class="mt-6">
            <div class="flex flex-wrap gap-2">
              @foreach($blog->tags ?? [] as $tag)
                <a class="inline-flex items-center gap-1.5 py-2 px-3 rounded-full text-sm bg-gray-100 text-gray-800 dark:bg-neutral-800 dark:text-neutral-200" href="#">
                  {{ $tag->tag_name }}
                </a>
              @endforeach
            </div>
