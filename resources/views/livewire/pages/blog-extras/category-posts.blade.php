<div>
    ...
    <div class="max-w-2xl text-center mx-auto mb-10 lg:mb-8 mt-6 md:mt-8 lg:mt-14">
        <h2 class="text-2xl font-bold md:text-4xl md:leading-tight dark:text-white">{{ $category->cat_name }}</h2>
        <p class="mt-1 text-gray-600 dark:text-neutral-400">{{ $category->cat_desc ?? '' }}</p>
    </div>
    <!-- End Title -->
    ...
    <div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
        ...
        <div class="grid lg:grid-cols-2 gap-6">
            @foreach ($postsCategories as $postCategory)
                <!-- Card -->
                <a wire:key="blog-{{ $postCategory->id }}" class="group relative block rounded-xl focus:outline-hidden" href="{{ route('page.blog.show', $postCategory->slug) }}">
                    ...
                    <div class="shrink-0 relative rounded-xl overflow-hidden w-full h-87.5 before:absolute before:inset-x-0 before:z-1 before:size-full before:bg-linear-to-t before:from-gray-900/70">
                        <img class="size-full absolute top-0 start-0 object-cover"
                            src="{{ asset(Storage::url($postCategory->featured_img)) }}"
                    ...
                    <div class="absolute top-0 inset-x-0 z-10">
                        <div class="p-4 flex flex-col h-full sm:p-6">
                            ...
                            <div class="flex items-center">
                                <div class="shrink-0">
                                    <img class="size-11 border-2 border-white rounded-full" src="{{ $postCategory->user->avatar_url ?? 'https://ui-avatars.com/api/?name=' . $postCategory->user->name . '&background=f44336&color=fff' }}" alt="{{ $postCategory->user->name }}">
                                </div>
                                <div class="ms-2.5 sm:ms-4">
                                    <h4 class="font-semibold text-white">{{ $postCategory->author }}</h4>
                                    <p class="text-xs text-white/80">{{ $postCategory->created_at->format('M d, Y') }}</p>
                                </div>
                            </div>
                            <!-- End Avatar -->
                        </div>
    ...
    <nav class="flex justify-between items-center gap-x-1 mt-6" aria-label="Pagination">
        ...
        <button
            ...
            @class([
                ...
                'disabled:opacity-50 disabled:pointer-events-none' => !$postCategory->onFirstPage()
            ])
            @disabled($postCategory->onFirstPage())
            aria-label="Previous"
        >
        ...
        <div class="flex items-center gap-x-1">
            @foreach ($postCategory->getUrlRange(1, $postCategory->lastPage()) as $page => $url)
                <button
                    wire:click="gotoPage({{ $page }})"
                    @class([
                        'min-h-9.5 min-w-9.5 flex justify-center items-center py-2 px-3 text-sm rounded-lg',
                        'text-gray-800 hover:bg-gray-100 focus:bg-gray-100 dark:text-white dark:hover:bg-white/10 dark:focus:bg-white/10',
                        'bg-gray-200 dark:bg-neutral-600' => $page === $postCategory->currentPage()
                    ])
                    aria-current="{{ $page === $postCategory->currentPage() ? 'page' : 'false' }}"
                >
                    {{ $page }}
                </button>
            @endforeach
        ...
        <button
            ...
            @class([
                'min-h-9.5 min-w-9.5 py-2 px-2.5 inline-flex justify-center items-center gap-x-2 text-sm rounded-lg',
                'text-gray-800 hover:bg-gray-100 focus:bg-gray-100 dark:text-white dark:hover:bg-white/10 dark:focus:bg-white/10',
                'disabled:opacity-50 disabled:pointer-events-none' => !$postCategory->hasMorePages()
            ])
            @disabled(!$postCategory->hasMorePages())
            aria-label="Next"
        >
